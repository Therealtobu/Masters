...
function module.SetConfigurationAsync(NewData)
	assert(type(NewData) == "table", "Configuration must be a table")

	local success, err = pcall(function()
		DataStore:SetAsync("Configuration", NewData)
	end)
...
function module.SetStationsAsync(NewData)
	assert(type(NewData) == "table", "Stations must be a table")

	local success, err = pcall(function()
		DataStore:SetAsync("Stations", NewData)
	end)

	if success then
		CurrentStations = NewData
		return true
	else
		warn("Stations:", err)
		return false
